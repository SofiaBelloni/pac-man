...
public class GhostFactoryImpl implements GhostFactory {
    ...
    private Pair<Integer, Integer> currentBlinkyPosition;
    ...
    public final Ghost blinky() {
        return new GhostAbstractImpl() {
            private final Pair<Integer, Integer> target = new Pair<Integer, Integer>(14, 14);
            public void create() {
                currentBlinkyPosition = new Pair<>(7, 6);
                blinkyDir = Directions.UP;
                isBlinkyRelaxed = true;
            }
            public Pair<Integer, Integer> getPosition() {
                return currentBlinkyPosition;
            }
            public int nextPosition(final Pair<Integer, Integer> pacManPosition) {
                for (Pair<Integer, Integer> p:map.keySet()) {
                    ...
                    if (isBlinkyRelaxed) {
                        ...
                        findPath(currentBlinkyPosition, pacManPosition);
                    }
                    if (currentBlinkyPosition.equals(pacManPosition)) {
                        return 1;
                    }
                    return 0;
            ...
            public boolean relax() {
                findPath(currentBlinkyPosition, target);
                if (currentBlinkyPosition.equals(target)) {
                    return false;
                }
                return true;
            }
            public void findPath(final Pair<Integer, Integer> position, final  Pair<Integer, Integer> targetPosition) {
                if (position.equals(targetPosition)) {
                    map.get(position);
                    move(position);
                    return;
                }
                if (position.equals(currentBlinkyPosition)) {
                    map.put(position, 0);
                }
                ...
                if (!setWall.contains(down) && down.getY() >= MIN && map.get(position) + 1 < map.get(down)) {
                    map.put(down, map.get(position) + 1);
                    findPath(down, targetPosition);
                }
                if (!setWall.contains(left) && left.getX() >= MIN && map.get(position) + 1 < map.get(left)) {
                    map.put(left, map.get(position) + 1);
                    findPath(left, targetPosition);
                }
               }
            public void move(final Pair<Integer, Integer> appo) {
                ...
                if (map.get(appo) == 1) {
                    currentBlinkyPosition = appo;
                    return;
                } else {
                    if (!setWall.contains(up) && up.getY() <= MAX && map.get(up) == map.get(appo) - 1) {
                        move(up);
                    }
                    if (!setWall.contains(down) && down.getY() >= MIN && map.get(down) == map.get(appo) - 1) {
                        move(down);
                    }
                    if (!setWall.contains(left) && left.getX() >= MIN && map.get(left) == map.get(appo) - 1) {
                        move(left);
                    }
